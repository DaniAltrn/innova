	...
	if($_SERVER["REQUEST_METHOD"]=="POST") 
	{
		if(check_admin_referer( 'edit-menu_', 'Rich_Web_Timeline_Nonce' ))
		{
			if(isset($_POST['rich_webTimeline_Save'])) 
			{
				...
				$wpdb->query($wpdb->prepare("INSERT INTO $ti_table_name_1 (id, rw_timeline_name, rw_timeline_style_id) VALUES (%d, %s, %s)", '', $rw_timeline_name, $rw_timeline_style));
			...
			else if(isset($_POST['rich_webTimeline_Update'])) 
			{
				...
				$wpdb->query($wpdb->prepare("UPDATE $ti_table_name_1 set rw_timeline_style_id = %s, rw_timeline_name = %s WHERE id = %d", $rw_timeline_style, $rw_timeline_name, $id));
...
<form method="POST" enctype="multipart/form-data">
	...
	<div style="position: relative; width: 99%; margin-right: 0; height: 50px;">
		<input type="button" class="AddTimeline" value="New Timeline" onclick="addTimeline(<?php echo $max_short_id[0]->id+1;?>)">
	...
	<div class="Rich_Web_Timeline_Content">
			...
			<table class="RW_TI_Table_2">
				<?php for($i=0;$i<count($get_timeline);$i++) {
					...
					<tr class="RW_TI_Table_2_Tr">
						<td><?php echo $i+1;?></td>
						<td><?php echo $get_timeline[$i]->rw_timeline_name;?></td>
						<td><?php echo $get_timeline_style[0]->rw_timeline_title;?></td>
						<td><?php echo count($get_timeline_event);?></td>
						<td onclick="rw_ti_copy(<?php echo $get_timeline[$i]->id;?>)"><i class="rw_ti_fileso rich_web rich_web-files-o"></i></td>
						<td onclick="rw_ti_edit(<?php echo $get_timeline[$i]->id;?>)"><i class="rw_ti_pencil rich_web rich_web-pencil"></i></td>
						<td onclick="rw_ti_del(<?php echo $get_timeline[$i]->id;?>)"><i class="rw_ti_trash rich_web rich_web-trash"></i></td>
					</tr>
				<?php } ?>
			</table>
		</div>
		<div class="Rich_Web_New_Timeline" style="display: none;">
			...
			<table class="Rich_Web_Timeline_SaveTable">
				...
				<tr class="Rich_Web_TI_SaveTable_TR">
					<td style="width: 50%;">
						<input type="text" class="Rich_Web_TI_Select_Menu Rich_Web_TI_ST_Name" name="rw_timeline_name" placeholder=" * Required" required="">
					</td>
					<td style="width: 50%;">
						<select class="Rich_Web_TI_Select_Menu Rich_Web_TI_ST_Style" name="rw_timeline_style">
							<?php for($i=0;$i<count($get_sstyle_options);$i++) { ?>
								<option value="<?php echo $get_sstyle_options[$i]->id;?>"><?php echo $get_sstyle_options[$i]->rw_timeline_title;?></option>
							<?php } ?>
						</select>
					</td>
				</tr>
				...
				<tr class="Rich_Web_TI_SaveTable_TR_1">
					<td style="width: 50%;">
						<select class="Rich_Web_TI_Select_Menu Rich_Web_TI_td_Icon_Input" style="font-family: 'FontAwesome', Arial;">
				...
				<tr class="Rich_Web_TI_SaveTable_TR_1">
					<td colspan="2">Image<br />(*) The image only add or edit in database.<br />(*) The image must be uploaded on the site. You can upload image from "Medios" &gt; "Biblioteca".</td>
				</tr>
				<tr class="Rich_Web_TI_SaveTable_TR_1">
					<td style="width: 50%;">
						<input type="text" class="Rich_Web_TI_Select_Menu Rich_Web_TI_td_DO_Image_input">
					</td>
                                        <td style="width: 50%;">
                                            <img id="Rich_Web_TI_td_DO_Image_input_view" src="" style="max-width: 200px; max-height: 200px;" />
                                        </td>
				</tr>
				...
				<tr>
					<td class="Rich_Web_TI_td_Data_Form" colspan="2" style="padding:0px;">
						...
						<input type="button" class="rich_web_Update_TI" style="display: none;" onclick="Rich_Web_Update_TI()" value="Update">
