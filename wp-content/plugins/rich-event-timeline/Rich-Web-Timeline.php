<?php
	...
	Plugin URI: http://www.rich-web.esy.es/wordpress-timeline
	...
	*/
	...
	function Rich_Web_Timeline_Widget() { register_widget('Rich_Web_Timeline_Widget'); }
	...
	function Rich_Web_Timeline_Admin_Init() {
		...
		wp_localize_script('rich-web-timeline-admin-scripts', 'object', array('ajaxurl' => admin_url('admin-ajax.php')));
	 	wp_enqueue_script('jquery');
		wp_enqueue_script('jquery-ui-core');
		wp_enqueue_script('jquery-ui-sortable');
		wp_enqueue_script('jquery-ui-datepicker');
		wp_enqueue_style('rich-web-timeline-admin-ui-css', 'http://ajax.googleapis.com/ajax/libs/jqueryui/1.8.21/themes/base/jquery-ui.css');
	...
	function Rich_Web_Timeline_Menu() {
	 	$complete_url = wp_nonce_url( '', 'edit-menu_', 'Rich_Web_Timeline_Nonce' );
	  	add_menu_page('Rich-Web Timeline Admin' . $complete_url, 'Timeline', 'manage_options', 'Rich-Web Timeline Admin' . $complete_url, 'rich_web_timeline_manager', plugins_url('/Images/rich-web-timeline-logo.png', __FILE__));
	  	add_submenu_page('Rich-Web Timeline Admin' . $complete_url, 'Rich-Web Timeline Admin' . $complete_url, 'Timeline Manager', 'manage_options', 'Rich-Web Timeline Admin' . $complete_url, 'rich_web_timeline_manager');
	  	add_submenu_page('Rich-Web Timeline Admin' . $complete_url, 'Rich-Web Timeline General' . $complete_url, 'Timeline Options', 'manage_options', 'Rich-Web Timeline General' . $complete_url, 'rich_web_timeline_general');
	  	add_submenu_page('Rich-Web Timeline Admin' . $complete_url, 'Rich-Web Products' . $complete_url, 'Our Products', 'manage_options', 'Rich-Web Products', 'rich_web_timeline_products');
	}

	function rich_web_timeline_manager() { require_once(dirname(__FILE__) . '/Rich-Web-Timeline-Admin-Manager.php'); }
	function rich_web_timeline_general() { require_once(dirname(__FILE__) . '/Rich-Web-Timeline-Admin-General.php'); }
	function rich_web_timeline_products() { require_once(dirname(__FILE__) . '/Rich-Web-Products.php'); }

	register_activation_hook(__FILE__,'Rich_Web_Timeline_Activate');
	function Rich_Web_Timeline_Activate() {	require_once(dirname(__FILE__) . '/Rich-Web-Timeline-Install.php'); }
