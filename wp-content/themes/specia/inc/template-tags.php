...
function specia_admin_init () {
    $title = sprintf(__( 'Specia Store'.' '.'<span class="update-plugins specia-counts"><span class="plugin-count">New</span></span>', 'specia' ) );

    require_once get_template_directory() . '/inc/specia-store.php';

    add_theme_page( $title, $title, 'edit_theme_options', 'specia-store', array( 'Specia_admin', 'settings_page' ) );
